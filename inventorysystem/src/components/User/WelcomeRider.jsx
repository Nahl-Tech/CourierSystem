...
function WelcomeRider() {
  ...
  // Establish socket connection with rider ID
  ...
  useEffect(() => {
    ...
    // Listen for the 'balanceUpdated' event from the server
    socket.on("balanceUpdated", (data) => {
      // Assuming `data.newBalance` contains the updated balance
      setRiders((prevRiders) => ({
        ...prevRiders,
        remaining_balance: data.newBalance,
      }));
      toast.info("Your balance has been updated.");
    });
  ...
  useEffect(() => {
    ...
    // Exclude orders with status 'delivered'
    ...
    if (dateFilter) {
      ...
      tempOrders = tempOrders.filter((order) => order.cutsomDate === selectedDate);
    }

    if (searchTerm) {
      tempOrders = tempOrders.filter((order) => {
        const trackingId = order.tracking_id || "";
        return trackingId.toString().toLowerCase().includes(searchTerm.toLowerCase());
      });
    }
  ...
  const riderLogout = () => {
    axiosInstance.post(import.meta.env.VITE_RIDER_LOGOUT).then(() => {
      ...
      toast.success("Rider Logged Out Successfully", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
          ...
          <div className="fixed inset-0 flex justify-center items-center z-50">
            ...
            <div className="relative mt-6 flex flex-col gap-5 bg-gradient-to-r from-rose-700 to-pink-600 rounded-xl px-8 py-6 text-white">
              ...
              <form className="w-full" onSubmit={updateStatus}>
                ...
                >
                  <option value={selectedOrder.status} selected>
        ...
        {showDetailModal && selectedOrder && (
          <div className="fixed inset-0 flex justify-center items-center z-50 p-5">
            ...
            <div
              ...
              className="h-[70vh] overflow-y-scroll relative max-w-lg w-full bg-gradient-to-r from-slate-900 to-slate-700 rounded-xl px-8 py-6 text-white shadow-lg shadow-black/30"
            >
              <button
              ...
              <div className="space-y-2">
                {/* Customer Details */}
                <div className="flex justify-between border-b-2 border-gray-500 pb-1">
        ...
        <div className="max-w-lg w-full bg-white shadow-md rounded-lg p-6 mb-4">
          ...
          <div className="mb-4 flex items-center">
            ...
            <button
              ...
              className="bg-blue-500 text-white py-1 px-4 rounded-md hover:bg-blue-600 transition duration-300 ease-in-out"
            >
              Clear
            </button>
          </div>
              ...
              filteredOrders.map((order, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-lg">
                      <span className="font-semibold">Name:</span> {order.cust_name}
                    </p>
                    ...
                    <p className="text-lg">
                      ...
                      <button
                        ...
                        className={`ml-2 text-[13px] px-1 py-0 rounded-lg uppercase ${buttonClass}`}
                      >
                        {order.status}
                      </button>
                    </p>
